...
```{r}
cowles.model <- glm(volunteer ~ 1 + neuroticism + extraversion, data = cowles, family = "binomial")
summary(cowles.model)
```
...
```{r}
cowles.boot <- Boot(cowles.model, f=coef, R = 5000) 
summary(cowles.boot) ## add the goals.model and stipulate how many resamples you want to run (typically 5,000)

confint(cowles.boot, level = .95, type = "norm") # add the bootstrap model you just created to this code
```
...
```{r}
exp(coef(cowles.model))

exp(cbind(OR = coef(cowles.boot), confint(cowles.boot)))
```
