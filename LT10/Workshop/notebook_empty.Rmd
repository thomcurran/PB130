...
```{r}
fit1.boot <- Boot(fit1, f=coef, R = 500) 
summary(fit1.boot)
...
```{r}
fit2.boot <- Boot(fit2, f=coef, R = 500) 
summary(fit2.boot)
...
```{r}
fit3.boot <- Boot(fit3, f=coef, R = 500) 
summary(fit3.boot)
...
Now we have an estimated population SD of 1.73, lets correct our correlation of -0.18 for range restriction. To do so, we can use the psych package, which implements Thorndike's case 2 using the rangeCorrection() function.
...
```{r eval = FALSE}
plot2 <- ggplot(perfectionism, aes(x = ??, y = ??)) + geom_point() + theme_classic()
plot2
```
