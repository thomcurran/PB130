...
```{r eval=FALSE}
rcorr(as.matrix(WHR[,c("??","??")], type="??")) # fill in the ?? gaps here for the explanatory variable and the correlation type (pearson)
```
...
```{r eval=FALSE}
happiness.model2 <- lm(??, data=WHR) # add ?? variables to the linear model code
