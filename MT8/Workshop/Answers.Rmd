...
WVS$V2A <- as_character(WVS$V2A) # this saves the country varaible as a "character" variable (i.e., the names of th countries) rather than its default "numeric" value.
```
...
```{r}
WVSjapan$predicted <- mean(WVSjapan$V55)
WVSjapan
```
...
```{r}
WVSjapan$residuals <- WVSjapan$V55 - WVSjapan$predicted
WVSjapan
```
...
```{r}
WVSjapan$residuals_squared <- WVSjapan$residuals^2
WVSjapan
```
...
```{r}
sum(WVSjapan$residuals_squared)
```
...
```{r}
Japan.model <- lm(V55 ~ NULL, data = WVSjapan) # enter the NULL model you are building
```
...
```{r}
anova(Japan.model) # enter the name of the model you have built usign lm()
```
...
```{r}
WVSjapan$Z <- scale(WVSjapan$V55)
WVSjapan
```


Use the Z-score equation to calcualte the Z score for a Jericho's value of 6:
...
WVSjapan %>% 
  ggplot(aes(x= Z)) +
  geom_histogram(aes(y=..density..), binwidth=.6, colour="black", fill="white") + 
  geom_vline(aes(xintercept=mean(Z)), col="blue", size=1) + 
  geom_vline(aes(xintercept=.122448), col = "red", size=1) +
  ylab("Number of Responses") +
  xlab("Perceptions of Choice") +
  ggtitle("Distribution of Perceptions of Choice Z Scores for Australia in the WVS with Blue Line Showing the Mean and Red Line Showing Jericho") +
  theme_minimal(base_size = 8)
