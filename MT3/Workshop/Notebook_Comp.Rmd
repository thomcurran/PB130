...
```{r}
favstats(~V10, data = WVS_short)
```
```{r}
favstats(~V11, data = WVS_short)
```
...
```{r}
Just_Iraq <- # into new dataframe
WVS_short %>% # from WVS_short
filter(V2 == "368") # Filter on the country code
Just_Iraq # check the new dataframe
```
...
```{r}
favstats(~ wellbeing, data = Just_Iraq)
```
