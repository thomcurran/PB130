---
title: "Workshop 2: Basics of Data Manipulaiton"
output:
  html_document:
    df_print: paged
...
You can access the .rdata (R data file) file of the data here: [INSERT LINK]. The file is called WV6_Data_R_v20180912.rds.

Download the file and save it in a logical place. I would recommend that you save it in your PBS folder under a folder called Workshop 2.
...
```{r}
WVS <- readRDS("../MT3/Workshop/WV6_Data_R_v20180912.rds")
```
...
The WVS seeks to help scientists and policy makers understand changes in the beliefs, values and motivations of people throughout the world. Thousands of political scientists, sociologists, social psychologists, anthropologists and economists have used these data to analyze such topics as economic development, democratization, religion, gender equality, social capital, and subjective well-being. You might remember from PB101 some trends that have been examined with this data.

Impresssive! So, what can we do to look at this data?

1. Doubleclick on it in the environment pane (right corner) to view it.
2. View()
3. Just type the dataframe name
...
Okay, still a bit messy, What about finding out the dimensions of the data frame with dim()?
...
You should see that the data frame contains the columns that match those in the WVS codebook (see: LINK).
...
If you said the minimum value -5 is outside of the range of values (1-4) then you are correct! This is becuase in the WVS not everybody answered this question or responded that they did not know. So rather than 1-4, the V10 variable ranges from -5 to 4, and includes (see codesheet here: [link]):
...
![alt text here](E:/Pictures/Piping.png)
...
So we have successfully removed the redundant information. Now lets hone in on this data. Let's say that in our project we are only interested in well-being within the USA. To do this, we need to create a new dataframe (Just_USA) from a subset of cases within the Short_WVS dataframe. Here we will use the V2 variable (country) and use the filter() function. Looking at the WVS codesheet tells us that the numeric code for USA in V2 is 840.
...
We could of course do the same for any other calculation. Try for instance using the mutate() fucntion to create a new variable called "product" that contains values of V10 multiplied by V11.

```{r}

```

Now we have created the wellbeing variable, we can address our focus aim, which is to understand levels of wellbeing in the USA. To do that, lets use the favstats() function to see what the average wellbeing score is for people in the USA
