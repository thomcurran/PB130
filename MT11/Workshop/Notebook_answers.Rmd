...
```{r}
WVSimmigrant$V39 <- revalue(as.factor(WVSimmigrant$V39), c("2"="0")) # revalue is part of the plyr package and takes the V39 variable from the WVSimmigrant data frame and replaces values of 2 with values of 0. This function always takes the input of a factor or categorical variable so that as.factor code just forces V23 to a factor (i.e., not a numeric variable).
```
...
```
As you can see, the groups are coded in the opposite direction (i.e., would not like immigrants -> would like immigrants) so the t value is -ve rather than +ve, but otherwise the output is exactly the same.
...
Okay so we are a little off in our rounding but the interpretation stays the same. If we randomly sampled the same data 100 times, the true population mean difference would fall somewhere between .01 and .46 in 95 of those samples. Thats all we can say. 
...
And well, what do you know! The bootstrap 95% confidence interval tells us something different to the normal theory 95% confidence interval. Here, we see that the range of values runs from -.0001 to .47 (your values might slightly differ to this, bootstrapping will produce slightly different estimates each time it resamples). Crucially, though, this range now includes zero! We cannot reject the null hypothesis.
...
WVSfinance <- 
  WVS %>% 
  select(V59,V23,V2A) # SELECT THE VARIABLES OF INTEREST HERE
WVSfinance
```
...
```{r eval=FALSE}
WVSfinance %>% 
  ggplot(aes(x= V59)) + ## fill in the variable gap here for financial satisfaction
  geom_histogram(aes(y=..density..), binwidth=1, colour="black", fill="white") + 
  geom_density(adjust = 4, alpha = .2, fill = "antiquewhite3") +
  ...
  xlab("satisfaction") + # add appropriate x lab
...
```{r eval=FALSE}
rcorr(as.matrix(WVSfinance[,c("V59","V23")], type="pearson")) # fill in the ?? gap here for the explanatory variable
```
...
Every 1 unit increase in financial satisfaction results in a .41 unit increase in life satisfaction 
...
Larger and expressed in units of SD
...
Finacial satisfaction explains a significant amount of empty variance in life satisfaction
...
```{r eval=FALSE}
confint(finance.model) # add the finance model to the confint() function
```
...
Range of likely values in the population using normal theory

Can we accept the null hypothesis?

No we reject the null

Why?

THe confidence interval does not include a zero effect
...
```{r eval=FALSE}
confint(finance.boot, level = .95, type = "norm") # add the bootstrap model you just created to this code
```
...
Range of likely values in the population using bootstrapping
...
A simple regression analysis was performed to test the relationship between financial satisfaction and life satisfaction among Americans in the World Values Survey. The regression model indicated that financial satisfaction explained 28% of the variance in life satisfaction (R = .53, R2 = .28, F(1) = 843.64, p < .001). Inspection of the model estimates indicated that financial satisfaction positively predicted life satisfaction (b = .41, β = .53, t (2208) = 29.05, p < .001). The 95% confidence interval associated with this estimate was obtained with 5,000 bootstrap resamples and did not include zero (95% CI = .38, .44).
