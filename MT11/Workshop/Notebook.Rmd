...
```{r}
WVS <- readRDS("C:/Users/tc560/Dropbox/Work/LSE/PB130/MT9/Workshop/WV6_Data_R_v20180912.rds")
...
To start, lets ask an interesting question (and quite topical) of the WVS. In the data, there are some items that ask respondants about their attitudes toward marginalised groups. One of those questions asks whether respondants:
...
Presently, the issue of attitudes toward immigrants is quite a 'live' one in the USA. So lets see whether Americans who reported that they would not like to have immigrants as neighbors differ in levels of life satisfaction than those who did not report such attitudes. Our theory is that life satisfaction may differentiate these attitudes, but we don;t know yet.
...
WVSimmigrant <- 
  WVS %>% 
  select(V2A, V39, V23) # SELECT THE VARIABLES OF INTEREST HERE
head(WVSimmigrant)
```
...
```{r}
favstats(~V23, V39, data=WVSimmigrant) # the second line of fav stats is our factor and it splits the SOP scores into a mean for North America and a mean for the UK.
```

Remember that those who mentioned they would not like to have immigrants as neighbors are codes as 0, all else are coded 1.
...
The difference is 0.24 points on a 1-10 Likert scale, which doesnt seem a great deal but in the context of the SD, could be significant. The question therefore is whether this mean difference is statistically meaningful or large enough for us to consider the possibility of a zero difference in the population unlikely.
...
To this point, I've been showing you how we the use the estiamted standard deviaiton of the sampling distribution - or the *standard error* - to gauge how far away our estiamte is from a zero effect. 

When we take the standard error and divide it by the estiamte we have a metric - a t ratio - that tells us how many standard errors our estimate is away from zero. If the t ratio is greater than 1.96, then we reject the null hypothesis and conclude that, assuming a normal sampling distribution, a zero difference is unlikely in the population.
...
95% confidence interval = estiamte +/- (SE*1.96)
...
Okay so we are a little off in our rounding but the interpretation stays the same. If we randomly sampled the same data 100 times, the true popualtion mean difference would fall somewhere betwen .01 and .46 in 95 of those samples. Thats all we can say! 

However, if zero is not inluded in that range then we can conclude that a zero mean difference is pretty unlikely.

Does this include zero? No! Therefore, the interpretation is that we can reject the null hypothesis. People who would not like to have immigrants as neighbors are still less satisfied with their life under this interpretation of staistical inference.
...
To get a clue of why the normal theory 95% confidence interval is too liberal (i.e., rejecting the null when it is possibly true), take a look at the distribution of life satisfaction above. We can see a negative skew meaning that a normal curve underestimates values below the mean, and overestimates them above the mean. These differences are marginal, but when the estimate is ONLY JUST significant, as is the case here, it can cause errorneous conclusions. This is why bootstrapping should always be preferred. It makes no assumptions about the distribution of variables and estimates.
...
## Task 1 - Select the variables and create a random sample
...
WVSfinance <- 
  WVS %>% 
  select(V23, V59, V2A) # SELECT THE VARIABLES OF INTEREST HERE
head(WVSfinance)
```
...
WVSfinance <- 
WVSfinance %>% 
filter(V59 >= "1" & V23 >= "1" & V2A == c("United States")) 
WVSfinance
```
...
Write the code to draw a plot of the distribution of the finance satisfaction. Given that the GINI coefficient is scaled between 0 and 1, change the binwidth to .05.

```{r eval=FALSE}
WVSfinance %>% 
  ggplot(aes(x= V59)) + ## fill in the variable gap here for GINI
  geom_histogram(aes(y=..density..), binwidth=1, colour="black", fill="white") + 
  geom_density(adjust = 4, alpha = .2, fill = "antiquewhite3") +
  geom_vline(aes(xintercept=mean(V59)), col="black", linetype="dashed", size=1) +
  geom_vline(aes(xintercept=median(V59)), col="black", size=1) +
...
```{r eval=FALSE}
rcorr(as.matrix(WVSfinance[,c("V59","V23")], type="pearson")) # fill in the ?? gaps here for the explanatory variable and the correlation type (pearson)
```
...
```{r eval=FALSE}
finance.model <- lm(V23 ~ 1 + V59, data=WVSfinance) # add ?? variables to the linear model code
...
```{r eval=FALSE}
lm.beta(finance.model) # add finance model to the lm.beta function
```
...
```{r eval=FALSE}
supernova(finance.model) # add finance model to the supernova function
```
...
```{r}
finance.boot <- Boot(finance.model, f=coef, R = 5000) 
summary(finance.boot)
```
...
```{r}
confint(finance.boot, level = .95, type = "norm")
```
