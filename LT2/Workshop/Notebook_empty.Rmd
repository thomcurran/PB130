...
Lets quickly run that calculation with our numbers from the supanova output 

```{r}
((.6040-.6008)/(2-1))/((1-.6040)/1492)
```

We have a F ratio of 12.06. F is a standardized metric with a smoothed distribution that can be used for Null Hypothesis Significance Testing. In order to know whether an F of 12.06 is large enough to be considered statistically significant we need to inspect a table of F distributions and find the critical value of F with 1,1492 degrees of freedom that cuts off 95% of the distribution. F tables are freely available online, one of them can be accessed here: https://web.stanford.edu/dept/radiology/cgi-bin/classes/stats_data_analysis/lesson_5/234_7_n.html

We can see that with 1 degree of freedom in the numerator and over 120 degrees of freedom in the denominator, we would need a F ratio larger than 6.6349 to consider the addition of gini statistically meaningful. Our F ratio is larger than this and therefore we prefer the full model - there is an improvement of fit with gini added but I would caution that it is small and significant only because of the very large sample size (making even small increases in R2 significant)
...
# Task 6 - Use this workshop for your quantitative writing practice due Wednesday but you have until next Monday to submit (01/02/2020)
