...
5. Plot simple relationships in data
...
We can see the median value is the same (2), but the means differ. There is also more spread in the Algerian data (larger SD). The numeric country code is fine but lets convert that country code to the country name.


```{r}
RandomSample$country <- countrycode(RandomSample$V2A, origin = "wvs", destination = "un.name.en") # this code takes the dataframe and uses the dollar sign $ to denote the name of the new column ($country) we are adding. The country code function then takes the country column in the existing dataframe ($V2A), the dataframe orign, which in this case is the "wvs", and then outputs the country in UN English name format "un.name.en". 
head(RandomSample) # check the data frame
```


Okay, now lets see those distributions in box plots overlayed with the data points using the layer geom_jitter.
...
Let's now get back to the original aim of this session and look at graphing wellbeing by country. For this, we need to return to our WVSwellbeing dataframe. First, lets code the countries with their charecter labels rather than numeric codes as we did above.


```{r}
WVSwellbeing$country <- countrycode(WVSwellbeing$V2A, origin = "wvs", destination = "un.name.en")
head(WVSwellbeing)
```


Notice ther warning here. A handful of codes couldn't be matched and have been replaced in the country column as NA. We'll return to this later but dont worry for now.

Next, lets summarise the happiness, health, and wellbeing variables by country. To do this, we are going to create a new dataframe (WVSsummary) that contains the mean happiness, health, and wellbeing scores for each country.
...
```{r}
WVSsummary %>% 
 # filter(country != is.na(country)) %>% # I'm filtering out those countries that couldn't be matched from their codes in the WVS
