...
```{r}
WVS <- readRDS("~/PB130-master/MT4/Workshop/WV6_Data_R_v20180912.rds")
...
WVSwellbeing %>% 
  ...
  ylab("Number of Responses") +
  xlab("Health")+
  ggtitle("Distribution of Health in the WVS") +
  theme_minimal(base_size = 8)
```
...
```{r eval=FALSE}
WVSsummary %>% 
  ggplot(aes(x = reorder(V2A, meanhappy), y = meanhappy)) + 
  geom_point() + 
  theme(axis.text.x = element_text(angle = 90, hjust = .5), text = element_text(size = 8)) + 
  xlab("Country") + 
...
```{r eval=FALSE}
WVSsummary %>% 
  ggplot(aes(x = reorder(V2A, meanhealth), y = meanhealth)) + 
  geom_point() + 
  theme(axis.text.x = element_text(angle = 90, hjust = .5), text = element_text(size = 10)) + 
  xlab("Country") + 
  ylab("Mean Reported Health Value") + 
  labs(title = "Self-reported health", 
       subtitle = "Source: World Values Survey, Wave 6, 2010-14")
```
