...
```{r}
bigfive <- read_csv("LT8/Lab/bigfive.csv")
bigfive
```
...
The net step after we have recoded the items is to check the scales are reliable. As we saw in the lecture, we can calculate the reilaibility of the Big 5 scales using the Cronbach's alpha, which is the ratio of variance in the true score to error variance. We saw in the lecture tha tit is calculated as:
...
Once we have established that the Big 5 scales are reliable, we can move to aggregating the items to create the variables for each sub-scale. We can easily do this with the mutate function, as we saw at the beginning of the first semester.
